...
describe('CheckRoster — validated at registration', () => {
...
describe('CheckRoster — the repository’s tier vocabulary, and a check nothing named', () => {
  it('refuses a tier outside the vocabulary, naming the check, its file and the valid tiers', () => {
    ...
    const reg = new CheckRoster({ tiers: ['fast', 'heavy'], originOf: () => '.specwarden/checks/x.check.mjs' });
  ...
  it('names no file when it does not know one, and accepts any tier when no vocabulary is given', () => {
    expect(() => new CheckRoster({ tiers: ['pr'] }).register(check({ id: 'x', tier: 'fast' }))).toThrow(
      'check \'x\' declares tier "fast" — expected one of: pr.',
    );
    expect(() => new CheckRoster().register(check({ id: 'x', tier: 'anything' }))).not.toThrow();
  });
