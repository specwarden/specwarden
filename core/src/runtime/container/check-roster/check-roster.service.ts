 ...
 * roster is built, against the config's vocabulary. It used to load: `tier: 'fastt'`
 * ran under `--all` and was in no tier a run selects — every `--tier` skipped it in silence,
 * which is a check nothing runs reporting nothing.
 */
...
export interface ICheckRosterOptions {
 ...
 */
export class CheckRoster {
  private readonly checks = new Map<string, ICheck>();

  constructor(private readonly options: ICheckRosterOptions = {}) {}
  ...
  /** The file a registered check came from, when the roster was told. */
