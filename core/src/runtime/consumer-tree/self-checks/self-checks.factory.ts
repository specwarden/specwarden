...
/**
 * What a consumer may tune about the self-checks. Every field is optional;
 ...
 */
export interface ISelfCheckOptions {
  ...
  /** Ids that count as enforcers beside the registered checks — perimeter policy ids,
  ...
  /**
   * The document that owns the self-checks’ own rule. Defaults to the consumer
   * directory's README, which `init` writes — so the owner resolves on day one.
   */
  readonly ruleOwner?: string;
}

export interface ISelfCheckInputs {
  readonly rules: () => readonly IRule[];
  /**
   * Whether the consumer DECLARED a rule register at all. `rules: []` is a declaration
   * (an empty register, audited as one); a config with no `rules` key is not, and the
   ...
   * built here, and a list read too early counts the self-checks as absent. */
...
/** The id of the rule the engine declares for its self-checks — doctor names it apart. */
export const SELF_CHECK_RULE_ID = 'self-checks-hold';

/**
 * The checks the engine runs on the declarations ITSELF, assembled from convention.
 *
 ...
 * set of self-checks with a hole in it and no way to see the hole.
 *
...
export function selfChecks(
  inputs: ISelfCheckInputs,
  options: ISelfCheckOptions = {},
  ...
  const ruleAudits: ICheck[] = [
    ...
    orphanCheck({ roster: inputs.roster, rules: inputs.rules, advisory: false, tier }),
    enforcementResolves({
  ...
  if (options.zone) {
    all.push(
      zoneBoundary({
        id: 'zone-boundary',
        title: 'a product source names no consumer literal and never imports the consumer zone',
        tier,
  ...
  for (const [id, why] of disabled) {
    if (!all.some((c) => c.id === id)) {
      notes.push(`self-check '${id}' is disabled but no self-check has that id — nothing was switched off.`);
      continue;
    }
    notes.push(`self-check '${id}' disabled: ${why}`);
  }

  const checks = all.filter((c) => !disabled.has(c.id));

  /**
   * The self-checks obey their own asymmetry.
   *
   ...
   * which is an engine reporting its own machinery as a defect: the worst possible
    ...
    ? [
        {
          ...
          enforcement: { enforcedBy: checks.map((c) => c.id) },
        },
      ]
    : [];
