...
import { SELF_CHECK_IDS, selfChecks } from './self-checks.factory';

const RULES: readonly IRule[] = [
  { id: 'r1', statement: 's', owner: 'README.md', enforcement: { enforcedBy: ['some-check'] } },
];

const inputs = (ids: readonly string[] = ['some-check']) => ({
  rules: () => RULES,
  rulesDeclared: true,
  roster: () => ids,
...
describe('the self-checks exist without being asked for', () => {
  it('builds the five that need no repository fact', () => {
    const { checks, notes } = selfChecks(inputs());
  ...
  it('runs them in the fast tier by default, and in the tier a repository names', () => {
    expect(selfChecks(inputs()).checks.every((c) => c.tier === 'fast')).toBe(true);
    expect(selfChecks(inputs(), { tier: 'pre-commit' }).checks.every((c) => c.tier === 'pre-commit')).toBe(true);
  });
  ...
  it('points the ratchet check at the conventional store under the consumer dir', () => {
    ...
    const a = selfChecks({ ...inputs(), consumerDir: '.specwarden' }).checks.find((c) => c.id === 'ratchet-direction');
    const b = selfChecks({ ...inputs(), consumerDir: '.consumer' }).checks.find((c) => c.id === 'ratchet-direction');
...
describe('the zone barrier is opt-in', () => {
  it('is absent unless a product tree is named — a barrier over nothing cannot fail', () => {
    expect(ids(selfChecks(inputs()).checks)).not.toContain('zone-boundary');
  });

  it('appears once one is', () => {
    const { checks } = selfChecks(inputs(), {
...
describe('switching one off is allowed and never silent', () => {
  it('removes the check and reports the reason', () => {
    ...
    expect(notes).toEqual(["self-check 'rule-coverage' disabled: rules are tracked in the forge"]);
  });

  it('reports an id that matches no self-check instead of swallowing it', () => {
    const { checks, notes } = selfChecks(inputs(), { disable: [{ id: 'rule-coverag', why: 'typo' }] });
  ...
  it('exposes the ids so a consumer can name one without guessing', () => {
    expect([...SELF_CHECK_IDS]).toEqual(expect.arrayContaining(['orphan-check', 'zone-boundary']));
  });
});
...
describe('a repository with no rule register yet is not punished for starting', () => {
  it('leaves out the four rule audits when `rules` was never declared, and says so', () => {
    const { checks, notes } = selfChecks({ ...inputs(), rulesDeclared: false });
    expect(ids(checks)).toEqual(['ratchet-direction']);
    expect(notes.some((n) => n.includes('no `rules` declared'))).toBe(true);
  });

  it('an EMPTY register is still a register — `rules: []` turns the audits on', () => {
    const { checks } = selfChecks({ ...inputs(), rules: () => [], rulesDeclared: true });
