...
/**
 * Evaluate a raw hook payload against a rule set — the pure core, so a test needs
 ...
 * about Claude. The rules — never force-push, never drop a volume — have nothing to
 * do with which model is typing, so a house running its own assistant supplies its
 * own runtime here and keeps every rule it already wrote.
 */
export function evaluatePayload(
  ...
  const verdict: IPerimeterVerdict = new PerimeterEngine(rules).evaluate(intent);
 ...
 * against the consumer's rules and exits 2 to block or 0 to allow. It FAILS OPEN on
 * every error path: a missing rules file, a malformed payload, a broken rule — all
