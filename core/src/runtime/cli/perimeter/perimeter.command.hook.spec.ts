 ...
 *
 * The rules below are plain objects rather than `commandRule(...)`: the temp directory
...
const RULE = `{
  ...
  evaluate: (intent) =>
    intent.command && intent.command.startsWith('rm ')
      ? { blocked: true, reason: 'rm deletes what review cannot see', ruleId: 'no-rm' }
      : { blocked: false },
}`;
...
describe('perimeter, the PreToolUse entry', () => {
  ...
  describe('it blocks what a rule names', () => {
    it('reads `export const rules`, blocks with 2 and tells the assistant why', async () => {
      declare(`export const rules = [${RULE}];`);
    ...
    it('reads a default-exported ARRAY of rules', async () => {
      declare(`export default [${RULE}];`);
      expect((await hook(RM)).code).toBe(2);
    });

    it('reads a default-exported object holding the rules', async () => {
      declare(`export default { rules: [${RULE}] };`);
      expect((await hook(RM)).code).toBe(2);
    });
    ...
    it('finds the declaration in its own folder, beside its test', async () => {
      ...
      declare(`export const rules = [${RULE}];`, 'perimeter/perimeter.mjs');
      expect((await hook(RM)).code).toBe(2);
    });
  ...
  describe('a runtime declared beside the rules is the one that speaks', () => {
    const RUNTIME = `{
      name: 'other',
      parse: (p) => (p && p.shell ? { tool: 'shell', command: p.shell } : null),
      exitCode: (v) => (v.blocked ? 7 : 0),
      formatBlock: (v) => 'DENIED ' + v.ruleId,
    }`;
    ...
    it('inside a default-exported object', async () => {
      declare(`export default { rules: [${RULE}], runtime: ${RUNTIME} };`);
      const r = await hook(JSON.stringify({ shell: 'rm -rf /' }));
      expect(r).toEqual({ code: 7, err: 'DENIED no-rm' });
    });
  });
  ...
  describe('it fails open on every fault', () => {
    ...
    it('allows when a rule throws, and still lets a later rule block', async () => {
      declare(`export const rules = [{ id: 'boom', evaluate: () => { throw new Error('x'); } }, ${RULE}];`);
      expect((await hook(LS)).code).toBe(0);
      expect((await hook(RM)).code).toBe(2);
    });

    it('allows when the file declares no rules at all', async () => {
