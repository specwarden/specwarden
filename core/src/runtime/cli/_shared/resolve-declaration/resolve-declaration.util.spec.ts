...
describe('declarationCandidates', () => {
  it('offers the flat path first and the folder-per-unit path second', () => {
    expect(declarationCandidates('/z', 'perimeter.mjs')).toEqual([join('/z', 'perimeter.mjs'), join('/z', 'perimeter', 'perimeter.mjs')]);
  });
  ...
  it('names the folder for the stem, so every dotted concern of one unit shares it', () => {
    ...
    expect(declarationCandidates('/z', 'dependency-pins.check.mjs')[1]).toBe(join('/z', 'dependency-pins', 'dependency-pins.check.mjs'));
  });
