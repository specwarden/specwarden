...
import { commandRule, writeRule } from './command-rule.factory';

/**
 * A perimeter rule is data plus one predicate; the factory supplies the parsing and
 * the block message. The message is what the agent reads INSTEAD of its action, so
 * it is asserted verbatim: a block that does not say which rule and where it is
...
describe('commandRule', () => {
  it('blocks a matching command, naming the hit, the rule and its owner', () => {
    const verdict = noForcePush({ owner: 'skills/git/SKILL.md' }).evaluate(bash('git push --force origin dev'));

    expect(verdict).toEqual({
      blocked: true,
      ruleId: 'no-force-push',
      reason: 'git push --force — rule no-force-push, owner skills/git/SKILL.md',
    });
  });

  it('appends the reason WHY when the rule states one', () => {
    const verdict = noForcePush({ owner: 'o.md', why: 'History others built on would vanish' }).evaluate(
      bash('git push --force'),
    );

    expect(verdict.reason).toBe(
      'git push --force — rule no-force-push, owner o.md. History others built on would vanish',
    );
  });

  it('still names the rule when it declares no owner', () => {
    expect(noForcePush().evaluate(bash('git push --force')).reason).toBe('git push --force — rule no-force-push');
  });
  ...
  /**
   * The rule reads SEMANTICS: an action with no command is not a shell call, whatever
  ...
  it('allows an empty command rather than handing the predicate nothing to read', () => {
    let called = false;
    const rule = commandRule({ id: 'r', match: () => ((called = true), 'hit') });

    expect(rule.evaluate(bash('   '))).toBe(ALLOW);
    expect(called).toBe(false);
  });
});

describe('writeRule', () => {
  const noGeneratedEdits = writeRule({
  ...
  it('blocks a write to a matching path, with the full reason', () => {
    expect(noGeneratedEdits.evaluate(write('db/meta.generated.json'))).toEqual({
      blocked: true,
      ruleId: 'generated-is-generated',
      reason:
        'db/meta.generated.json — rule generated-is-generated, owner skills/structure/SKILL.md. Regenerate it instead',
    });
  });
