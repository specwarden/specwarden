...
const declared = (): readonly ICheck[] => [
  ...(config.checks ?? []),
  // `checks` is optional on a plugin: without the fallback, one that contributes none put
  // `undefined` into the registry — which only a typecheck of this file could see, and
  // until it moved into a package nothing typechecked it.
...
describe('the whole workspace, in one config', () => {
  it('every package contributes checks, and no two of them collide on an id', () => {
    ...
    // The same list the CLI scenes hold the file-based config to.
    expect([...ids].sort()).toEqual([...EVERY_CHECK_ID]);
  });
  ...
  it('the broken repository is caught by EACH package, not merely by one of them', async () => {
    ...
    // EQUAL, not "contains": a defect for one package must not light up another's check.
    expect([...failed].sort()).toEqual([...CAUGHT_IN_BROKEN]);
  });
