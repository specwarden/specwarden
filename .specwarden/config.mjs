 ...
 *
 * WHY THE ENGINE IS ITS OWN FIRST CONSUMER. Every argument specwarden makes is about
...
export default defineConfig({
   ...
   * package rather than a tier nobody watches.
   */
  tiers: ['fast', 'heavy'],
   ...
   * because a repository whose checks run on a build of itself has no honest way to
  ...
  sharedBuildInputs: [
    ...
    { prefix: 'core/src/', why: 'the engine running the checks is the thing that changed' },
  ],
  ...
  selfChecks: {
     ...
     * its own checks. The engine is installed into repositories that have none of them,
    ...
    zone: {
      ...
      forbiddenLiterals: [
        ...
        { label: 'this repository’s own checks', pattern: /checks\/repository\// },
      ],
    },
  },
});
