...
| File                | What                                                           |
| ------------------- | -------------------------------------------------------------- |
| `config.mjs` | tiers, shared build inputs, the zone barrier over `core/src`   |
| `rules.mjs`         | the rules no single check owns, and the ones nothing can check |
| `checks/`           | one file per family of checks; a file here IS a check          |

## Why the engine is its own first consumer
...
its own test. The files here **wrap** it. A check whose logic is inline is a check whose
